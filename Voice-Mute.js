...
module.exports = {
  conf: {
    aliases: ["voicemute", "voice-mute", "vmute", "smute", "seslimute", "seslisustur"],
    name: "Vmute",
    help: "vmute <kullanıcı> <süre> <sebep> Ses kanallarında mute atmayı sağlar.",
  },
  ...
  run: async (client, message, args, embed) => {
    ...
    if (conf.penals.voiceMute.roles.some(x => member.roles.cache.has(x))) return message.channel.error(message, "Bu üye zaten susturulmuş!"), message.react(emojis.cross);
    const duration = args[1] ? ms(args[1]) : undefined;
    if (!duration) return message.channel.error(message, `Geçerli bir süre belirtmelisin!`), message.react(emojis.cross);
    const reason = args.slice(2).join(" ") || "Sebep Belirtilmedi!";
    if (!message.member.hasPermission(8) && member.roles.highest.position >= message.member.roles.highest.position) return message.channel.error(message, "Kendinle aynı yetkide ya da daha yetkili olan birini susturamazsın!"), message.react(emojis.cross);
    if (!member.manageable) return message.channel.error(message, "Bu üyeyi susturamıyorum!");
    if (conf.penals.voiceMute.limit > 0 && vmuteLimit.has(message.author.id) && vmuteLimit.get(message.author.id) == conf.penals.voiceMute.limit) return message.channel.error(message, "Saatlik susturma sınırına ulaştın!"), message.react(emojis.cross);
    ...
    message.channel.send(embed.setDescription(`${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **sesli kanallarda** susturuldu! \`(Ceza Numarası: #${penal.id})\``)), message.react(emojis.mark);
    ...
    const log = new MessageEmbed()
      ...
      .setColor("RED")
      .setDescription(`
${member.toString()} üyesi, \`${time}\` boyunca **sesli kanallarda** susturuldu! (Ceza Numarası: \`#${penal.id}\`)
