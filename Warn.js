...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    if (!message.member.hasPermission(8) && !conf.penals.warn.staffs.some(x => message.member.roles.cache.has(x))) return message.channel.error(message, "Yeterli yetkin bulunmuyor!"), message.react(emojis.cross);
    ...
    if (!member.manageable) return message.channel.error(message, "Bu üyeyi susturamıyorum!"), message.react(emojis.cross);
    ...
    message.channel.send(embed.setDescription(`${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle susturuldu! \`(Ceza Numarası: #${penal.id})\``)), message.react(emojis.mark);
    ...
    const log = new MessageEmbed()
      ...
      .setColor("RED")
      .setDescription(`
${member.toString()} üyesi sunucudan uyarıldı! (Ceza Numarası: \`#${penal.id}\`)
