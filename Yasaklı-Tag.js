...
module.exports = {
  conf: {
    aliases: ["yasaklı-tag", "bannedtag", "yasaklıtag"],
    name: "Yasaklı-Tag",
    help: "yasaklıtag <ekle/sil> <tag> / <say/liste> Yasaklı tag ekleme, silme, listeleme görevi sağlar.",
  },
  ...
  run: async (client, message, args, embed) => {
    ...
    if (["ekle", "add"].includes(args[0])) {
      ...
      const filtered = message.guild.members.cache.filter((x) => x.user.username.includes(args[1]) && !conf.penals.jail.roles.some((a) => x.roles.cache.has(a)));
      ...
      filtered.forEach(async (x) => {
        await x.setRoles(conf.penals.jail.roles);
